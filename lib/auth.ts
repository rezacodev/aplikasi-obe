...
export async function getServerAuthSession() {
  return await getServerSession(authOptions as any) // eslint-disable-line @typescript-eslint/no-explicit-any
}

export function hasRole(session: Session | null, role: string): boolean {
  return (session as any)?.user?.roles?.includes(role) || false // eslint-disable-line @typescript-eslint/no-explicit-any
}

export function hasPermission(session: Session | null, permission: string): boolean {
  return (session as any)?.user?.permissions?.includes(permission) || false // eslint-disable-line @typescript-eslint/no-explicit-any
}
